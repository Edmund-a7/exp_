...
class DummyLLMAgentSceneOnly(DummyLLMAgent):
    def process_prompt(self, prompt, prompt_id, global_registry):
        return [], {}, ["snowy forest", "pine trees"]
...
class DummyFrameInfo:
    def __init__(self, entity_score: float, scene_score: float) -> None:
...
class DummyMemoryBankEviction:
    ...
    def select_frame_from_chunk(self, **kwargs):
        self.frame_archive["p2_c3_f0"] = DummyFrameInfo(entity_score=0.1, scene_score=0.9)
    ...
    def update_scene_memory(self, frame_id, scene_score):
        self.update_scene_calls.append((frame_id, scene_score))
...
def test_reset_agent_state_resets_iam_bank_length():
    ...
    pipeline.current_scene_texts = ["forest"]
    ...
    assert pipeline.current_scene_texts == []
...
def test_process_prompt_start_retrieves_scene_memory_without_entities():
    AgentCausalInferencePipeline = _load_agent_pipeline_class()
    pipeline = AgentCausalInferencePipeline.__new__(AgentCausalInferencePipeline)
    ...
    assert pipeline.current_entities == []
    assert pipeline.current_scene_texts == ["snowy forest", "pine trees"]
    assert pipeline.agent_memory_bank.retrieve_calls == [([], ["snowy forest", "pine trees"])]
    assert pipeline.injected is True


def test_process_chunk_eviction_runs_with_scene_only_context():
    AgentCausalInferencePipeline = _load_agent_pipeline_class()
    pipeline = AgentCausalInferencePipeline.__new__(AgentCausalInferencePipeline)
    pipeline.current_prompt_id = 2
    pipeline.current_chunk_id = 3
    pipeline.current_entities = []
    pipeline.current_scene_texts = ["snowy forest"]
